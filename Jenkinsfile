...
pipeline {
    ...
    environment {
        // ИСПРАВЛЕНО: Убраны лишние пробелы в конце URL
                ...
                script {
                    ...
                    sh """
                        ...
                        if ! docker ps --format '{{.Names}}' | grep -q "^${containerName}\\$"; then
                            echo "❌ Контейнер ${containerName} не найден!"
                            docker ps -a --filter name=${containerName}
                            exit 1
                        fi
                        echo "✓ Контейнер запущен."

                        # 2. Проверка внутри контейнера
                        MAX_RETRIES=10
                        RETRY_COUNT=0
                        while [ \$RETRY_COUNT -lt \$MAX_RETRIES ]; do
                            # Используем || true, чтобы скрипт не падал сразу, а давал шанс на повтор
