...
pipeline {
    ...
    environment {
        // URL репозитория с ПРИЛОЖЕНИЕМ (исправлено: без пробелов!)
        ...
        // Порт 8090 (8080 — Jenkins, 8081 — hw25-web)
        DEPLOY_PORT = '8090'
        REPORT_DIR = 'reports'
    ...
    stages {
        ...
        // 🔥 ИСПРАВЛЕНО: Убрана проблемная конструкция grep с экранированием $
        stage('Health Check') {
            steps {
                ...
                script {
                    ...
                    sh """
                        ...
                        # 1. Проверяем, запущен ли контейнер (через точное совпадение имени)
                        RUNNING=\$(docker ps --format '{{.Names}}' | grep -x "${containerName}" || true)
                        if [ -z "\$RUNNING" ]; then
                        ...
                        # 2. Проверка внутри контейнера (самый надежный способ)
                        MAX_RETRIES=10
                        RETRY_COUNT=0
...
}  // 👈 ВОТ ЭТОЙ СКОБКИ НЕ ХВАТАЛО! Закрывает pipeline {